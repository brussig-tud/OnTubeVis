...
std::tuple<float, float> layer_compile_state::get_segment_local_time_and_arc_length(segment_time<float> segtime, float t) const {
	...
	float s = otv::arclen::eval(arc_lengths[global_segment_index], t_local);
	return { t_local, s };
}
...
std::vector<layer_compile_result> glyph_compiler::compile_glyph_attributes(
	...
	const otv::arclen::parametrization& parametrization,
...
//		for(int layer_idx = 0; layer_idx < layer_count; ++layer_idx) {...}
	...
	for(const auto& layer_config : layers_config.layer_configs) {
		...
		if(!layer_config.mapped_attributes.empty()) {
			layer_compile_result layer_result = compile_glyph_layer(data_set, parametrization, data_set.get_attribute_names(), position_attribute, layer_config);

			/* sanity check */ {
				const size_t stride_check = layer_result.glyphs.get_glyph_attribute_count();
				const size_t stride = layer_result.glyphs.size() / layer_result.timestamps.size();
				assert(stride == stride_check);
				assert(layer_result.timestamps.size() == layer_result.glyphs.size() / stride);
			}

			res.push_back(layer_result);
		}
	}
	...
	while(state.has_trajectory()) {
		...
		while(state.has_segment()) {
			...
			if(segment.is_valid()) {
				...
				state.place_glyph(layer_config, attribute_values, segment.s, min_t, length_scale, include_hidden_glyphs);
			} else {
	...
	while(state.has_trajectory()) {
		...
		while(state.has_segment()) {
			...
			if(segment.is_valid()) {
				...
				state.place_glyph(layer_config, interpolation.get_attribute_values(), segment.s, sample_t, length_scale, include_hidden_glyphs);
			}
...
layer_compile_result glyph_compiler::compile_glyphs_front_equidistant(
	...
	while(state.has_trajectory()) {
		...
		while(state.has_segment()) {
			...
			const float sample_t_local = otv::arclen::map(parametrization.t_to_s[next_global_segment_index], parametrization.s_to_t[next_global_segment_index], sample_s);

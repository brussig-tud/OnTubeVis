...
//***** begin interface of surface_mod.glsl ***********************************
struct Material {
	int brdf_type;
...
vec4 compute_reflected_appearance_texture(vec3 position_eye, vec3 normal_eye, vec2 texcoords, vec4 color, int side, float specular_factor);
...
vec4 compute_reflected_appearance(vec3 position_eye, vec3 normal_eye, vec4 color, int side, float specular_factor)
{
	...
	Material M = material;
	M.specular_reflectance *= specular_factor;

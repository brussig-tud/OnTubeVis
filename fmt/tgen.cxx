...
struct tgen<flt_type>::Impl {
	...
	)
	{
		if (cgv::utils::to_lower(fields[0]).compare(name) == 0)
		{
			...
			{
				if (*visited)
					std::cerr << "[fmt::tgen] WARNING: '"<<name<<"' specified more than once! Using new value: "<<*out << std::endl;
				*visited = true;
			}
			else
				std::cerr << "[fmt::tgen] WARNING: '"<<name<<"' specification could not be parsed! Line: "<<line << std::endl;
...
template <class flt_type>
const visual_attribute_mapping<flt_type> tgen<flt_type>::Impl::attrmap({
...
template <class flt_type>
const std::string& tgen<flt_type>::format_name (void) const
{
...
template <class flt_type>
bool tgen<flt_type>::can_handle (std::istream &contents) const
{
	std::string str;
	...
	{
		//std::cout << "[fmt::tgen] first line in stream must be \"TGEN v\"+version, but found \"" << str << "\" instead!" << std::endl;
		return false;
	}
...
template <class flt_type>
traj_dataset<flt_type> tgen<flt_type>::read (
	       ...
	       && Impl::read_next_nonempty_line(&line, &tokens, whitespaces, quotations, contents, &fields))
	{
		...
		// unknown declaration, print warning
			...
			std::cerr << "[fmt::tgen] WARNING: unknown declaration: '"<<fields[0]<<"', ignoring" << std::endl;
	}

	// post-process parsed input
	if (!num_specified && !segs_specified)
	{
		std::cerr << "[fmt::tgen] ERROR: not all necessary properties specified!" << std::endl;
...
template struct tgen<float>;
//template struct tgen<double>;
...
cgv::base::object_registration<tgen<float> > flt_tgen_reg("tgen trajectory handler (float)");
//cgv::base::object_registration<tgen<double> > dbl_tgen_reg("tgen trajectory handler (double)");

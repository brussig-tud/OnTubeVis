...
/// Controls logging verbosity. Higher values mean more messages.
...
OTV_API OTV_VisSetupHandle otv__create_VisSetup (const char *const name) {
	...
	if constexpr (log_level > 0)
		std::clog << "otv__create_VisSetup: created setup "<<hex(new_vs)<<std::dec<<" named '"
		          <<new_vs->name<<"'."<<std::endl;
...
OTV_API bool otv__add_layer (OTV_VisSetupHandle vis_setup, const OTV_LayerConfig *config)
{
	...
	if constexpr (log_level > 0)
		std::clog << "otv__add_layer: added layer of type '"<<otv__string_from_GlyphType(new_layer.type)
		          <<"'" << std::endl

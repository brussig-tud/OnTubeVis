...
bool optixtracer_textured_spline_tube_builtin::update_accelds (const traj_manager<float>::render_data *render_data)
{
	...
	/* local scope */ {
		...
		constexpr float mscale = 1; // <-- hand-tuned tangent scaling factor (we likely won't need it)
		for (const auto &ds : render_data->datasets) for (const auto &traj : ds.trajs)
		{
			...
			for (unsigned i=0; i<num_segs; i++)
			{
				...
				positions.emplace_back(to_float3(b0-(b1-b0)));
				radii.emplace_back(br0-(br1-br0));
				positions.emplace_back(to_float3(b1));
				radii.emplace_back(br1);
				positions.emplace_back(to_float3(b2+(b2-b1)));
				radii.emplace_back(br2+(br2-br1));
				// - 2nd segment
				indices.emplace_back(unsigned(positions.size()));
				positions.emplace_back(to_float3(g0-(g1-g0)));
				radii.emplace_back(gr0-(gr1-gr0));
				positions.emplace_back(to_float3(g1));
				radii.emplace_back(gr1);
				positions.emplace_back(to_float3(g2+(g2-g1)));
				radii.emplace_back(gr2+(gr2-gr1));
			}
		}
	}

...
struct stream_ds_helper : public traj_format_handler<float>
{
	...
	static traj_attribute<float>& add_streaming_dummy_attrib (
		traj_dataset<float> &dataset, const std::string &attrib_name, const unsigned buffer_size,
		const cgv::vec2 &vattrib_range
	){
		// create new attribute representing the stream
		const auto id = (unsigned)attributes(dataset).size();
		const std::string name = attrib_name.empty() ? "stream"+std::to_string(id) : attrib_name;
...
void on_tube_vis::start_new_streaming_session (const VisSetup &vis_setup)
{
	...
	{
		switch (i) {
			case 0: return VAttrib::Value0;
			case 1: return VAttrib::Value1;
			case 2: return VAttrib::Value2;
			case 3: return VAttrib::Value3;
			default: /* crash_below() */;
		}
		throw std::runtime_error("invalid vattrib index");
	};
	// - helper function to either add a new dummy attrib with the given name and other properties or reference an
	//   existing one if one with the same name already exists
	const auto get_or_add_dummy_attrib = [](
		traj_dataset<float> &dataset, const std::string &attrib_name, const unsigned buffer_size,
		const cgv::vec2 &vattrib_range
	) -> const traj_attribute<float>& {
		const auto &const_ds = dataset;
		auto it = const_ds.attributes().find(attrib_name);
		if (it == const_ds.attributes().end())
			return stream_ds_helper::add_streaming_dummy_attrib(dataset, attrib_name, buffer_size, vattrib_range);
		return it->second;//*/throw std::runtime_error("FATAL ERROR: duplicate attribute name!!!");
	};
	...
	for (unsigned l=0; l<vis_setup.layers.size(); ++l)
	{
		// convenience statics
		static const vec2 range01{0, 1}, range_m11{-1, 1}, range02(0, 2);

		// convenience shorthands
		const auto &layer_src = vis_setup.layers[l];
		const auto &attrib_sources = vis_setup.layer_sources[l];
...
void on_tube_vis::update_grid_ratios (void) {
	...
	if (! client.data->datasets.empty()) {
		...
		for (const auto &ds : client.data->datasets) {
			num += ds.irange.n;
...
void on_tube_vis::update_attribute_bindings(void)
{
	...
	if (traj_mgr.has_data()) {
		...
		playback.follow_traj = std::min(
			playback.follow_traj, (unsigned)client.data->datasets[0].trajs.size()-1
		);
		...
		unsigned node_indices_count = (unsigned)client.data->indices.size();
		...
		for (const auto &dataset : traj_mgr.datasets())
		{
			...
			for (const auto &trajectory : dataset.trajectories(pos_attrib))
			{
				...
				// Each trajectory has one fewer segment than nodes, so subtracting the number of
...
void on_tube_vis::update_debug_attribute_bindings() {
	...
	if(traj_mgr.has_data()) {
		...
		if(debug.render_mode == DebugRenderMode::kDisabled) {
			...
			const size_t num_nodes = client.data->positions.size();
			for(size_t i = 0; i < num_nodes; i++) {
				// convenience shortcut
				const auto &col = client.data->colors[i];

				nodes.add(client.data->positions[i], col, client.data->radii[i]);
				segments.add_position(client.data->positions[i]);
				segments.add_radius(radius_scale * client.data->radii[i]);
			...
			nodes.indices = client.data->indices;
...
void on_tube_vis::calculate_bounding_box(void) {
	...
	if(traj_mgr.has_data()) {
		// ensure render data
		client.data = &(traj_mgr.get_render_data());
		auto& positions = client.data->positions;
		auto& tangents = client.data->tangents;
		auto& radii = client.data->radii;
		auto& indices = client.data->indices;
...
void on_tube_vis::create_density_volume(context& ctx, unsigned resolution) {
	...
	if(voxelize_gpu) {
		...
		// if(node_idx_buffer_ptr)
		// 	density_volume.compute_density_volume_gpu(ctx, client.data, render.style.radius_scale, *node_idx_buffer_ptr, client.render_sbo, density_tex);
	} else {
		density_volume.compute_density_volume(client.data, render.style.radius_scale);
...
void on_tube_vis::draw_trajectories(context& ctx)
{
...
#endif
	{
		...
		if (!render.style.forward) fbc.enable(ctx, false);
		...
		if(render.sorter.is_initialized() && debug.sort && do_sort && !debug.force_initial_order) {
			...
			sort_arguments.bind_buffer("node_index_buffer", *node_idx_buffer_ptr);
			// measure sort time
		...
		if (!render.style.forward) fbc.disable(ctx, false);
...
#ifdef RTX_SUPPORT
	else { // if optix.enabled && optix.initialized
		// delegate to OptiX raytracing
		optix_draw_trajectories(ctx);
	...
	if (!(optix.enabled && optix.initialized && optix.debug))
#else
	...
	{ // Deferred shading:
		...
		// Set uniforms.
		deferred_prog.set_uniform(ctx, "length_scale", render.style.length_scale);
		deferred_prog.set_uniform(ctx, "antialias_radius", render.style.antialias_radius);

		ctx.set_material(render.style.material);
		deferred_prog.set_uniform(ctx, "map_color_to_material", static_cast<int>(render.style.map_color_to_material));
		deferred_prog.set_uniform(ctx, "culling_mode", static_cast<int>(render.style.culling_mode));
		deferred_prog.set_uniform(ctx, "illumination_mode", static_cast<int>(render.style.illumination_mode));
		...
		#endif
		deferred_prog.set_uniform(ctx, "viewport_width", static_cast<float>(viewport[2]));
		const auto fb_size = fbc.get_size();
		deferred_prog.set_uniform(ctx, "framebuf_width", static_cast<float>(fb_size.x()));
